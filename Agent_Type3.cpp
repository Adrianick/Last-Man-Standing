...
Agent_Type3::Agent_Type3() : Agent(80, 30, 1, 0) {}

void Agent_Type3::itemEquip(const Item *item) {
...
/// Agentul 3 cauta sa se lupte mai presus de orice, din cauza asta, daca nici un agent nu e in raza lui de actiune
/// o sa verifice doar  daca pozitia in care se indreapta este  == 0 (goala) sau else (un item)
int
Agent_Type3::moveAgent(int mapOfTheGame[15][15], std::map<int, Agent *> const &agents, std::map<int, Item *> &items) {
    ...
    int lowestDmgAgent, linieLow, coloanaLow;
    bool sfarsit = false;
    for (int l = -1 * speed; l <= 1 * speed; l++) {  /// Verifica tot imprejurul pentru iteme sau agenti
        for (int c = -1 * speed; c <= 1 * speed; c++) {
            ...
            if (mapOfTheGame[pozitieX + l][pozitieY + c] > 0) {
                if (linie == 0) {
                    linie = 1;
                    lowestDmgAgent = agents.find(mapOfTheGame[pozitieX + l][pozitieY + c])->second->getDamage();
                    linieLow = l;
                    coloanaLow = c;
                }
                ...
                if (agents.find(mapOfTheGame[pozitieX + l][pozitieY + c])->second->getDamage() < lowestDmgAgent) {
                    lowestDmgAgent = agents.find(mapOfTheGame[pozitieX + l][pozitieY + c])->second->getDamage();
                    linieLow = l;
                    coloanaLow = c;
                }
    ...
    if (!sfarsit)
        if (linie) { /// cauta agentul cu damage-ul cel mai mic si il ataca
